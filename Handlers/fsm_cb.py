...
async def who_fun(cb: CallbackQuery, state: FSMContext):
    ...
    await safe_delete(cb.message)
    await cb.bot.send_message(cb.from_user.id, text=record_buttons[lang]['steps']['name'])
...
async def name(m: Message, state: FSMContext):
    ...
    lang = get_user_language(user_id)
    if m.text and m.text.replace(" ", "").isalpha():
...
async def phone(m: Message, state: FSMContext):
    user_id = m.from_user.id
    lang = get_user_language(user_id)
    if not m.text or not is_valid_phone(m.text):
        await safe_delete(m)
